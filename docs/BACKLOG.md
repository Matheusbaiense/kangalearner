...
- [x] **Per-state HPT/PDA prep hubs**: /hpt and /pda now render per-jurisdiction from `apps/web/src/lib/statePrepHubs.ts` (hazard-test format/when/cost + practical-test format/routes/bring for all 8, researched officially and adversarially verified 2026-08-17). NT's hazard hub honestly says the NT has no hazard test. Research also corrected `stateResources`: QLD's hazard test has been learner-stage since 29 March 2021.
