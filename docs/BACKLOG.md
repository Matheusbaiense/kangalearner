...
- [ ] **`/journey`, `/supervisor`, `/overseas-licence` per state**: still WA-written guides (reachable from /resources only when WA is selected). Journey/supervisor can reuse `stateResources.ts` hours + steps; overseas-licence needs per-state recognition lists — a real research task.
