...
- [ ] **`/resources` is still WA-only**: 43 WA-specific references with a "WA resources for now" disclaimer. Needs per-state links (authority, booking, handbook) now that 8 jurisdictions are live — `stateProfiles.ts` already holds authority + handbook URL per state.
