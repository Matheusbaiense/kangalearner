...
Migrações em `supabase/migrations/` (001–034). 013 cria `user_settings` antes das policies RLS (bases novas); 018 mantém `CREATE TABLE IF NOT EXISTS`. 027 só altera `update_updated_at()` (001); `set_updated_at` era typo. 029 revoga `rls_auto_enable` só se a função existir (drift de prod). **Nunca reaplicar 013/027 em prod.** 031 relock `user_category_stats` SELECT-only + guard no RPC; 032 protege colunas sensíveis de `profiles`; 033 revoga grants client no marketplace scaffold; 034 hygiene (profiles SELECT unificado, saved_questions granular, initplan `auth.uid()`, search_path). **Staging (`zlsaerfsrfyxpbpxorwo`): 001–034 aplicadas 2026-08-18; smoke RLS a–f + 034 PASS (fixtures `smoke-a@` / `smoke-b@`).** Preview: `NEXT_PUBLIC_SUPABASE_*` → staging (2026-08-19); falta `SUPABASE_SERVICE_ROLE_KEY` de Preview. Playwright 6.2: `e2e/auth-staging.spec.ts` + `E2E_STAGING_*` (nunca prod). RLS 6.3: `pnpm rls:staging` (skip sem credenciais). Prod ainda sem 031–034. Backup `main` verde após PR #198.
