...
| 0.5 | Sentry DSN em Vercel Production | `SENTRY_DSN` + `NEXT_PUBLIC_SENTRY_DSN` no `vercel env ls` | [~] | Claude 2026-08-19: DSN recriado (95 chars), redeploy `dpl_GUtspk7mE8TiVpcSwtro9669e3j5` Ready, client `captureException` + `flush() true`. Dashboard Sentry ainda sem login nesta janela. | Rota temporária `GET /api/sentry-test` (3/h/IP) para o evento server. Remover depois do aceite. Sem colar DSN. |
