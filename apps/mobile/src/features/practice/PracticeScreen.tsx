...
import { CATEGORIES, categoryLabel, correctLetter, questionsForState, tx, type AnswerRecord } from "../../lib/questions";
import {
  enqueueSync,
  loadAnswers,
  loadSavedQuestions,
...
export function PracticeScreen() {
  ...
  async function choose(questionId: string, letter: string) {
    ...
    await enqueueSync({ type: "attempt", id: `${questionId}:${record.answeredAt}`, payload: record });
  }
        ...
        filtered.map((q) => {
          ...
          return (
            <Card key={q.id}>
              ...
              <Text selectable style={{ color: c.ink, fontWeight: "900", fontSize: 21, lineHeight: 30 }}>
                {tx(q.q, uiLang)}
              </Text>
              ...
              <Text style={{ color: c.muted, fontWeight: "800" }}>{categoryLabel(q.cat, uiLang)}</Text>
              <View style={{ gap: spacing.sm }}>
                {q.opts.map((option) => {
                  const picked = answer?.chosen === option.l;
                  const correct = Boolean(option.ok);
                  const show = Boolean(answer);
                  const borderColor = show && correct ? colors.success : show && picked ? colors.danger : c.border;
                  return (
                    <Pressable
                      key={option.l}
                      accessibilityRole="button"
              ...
              {answer ? (
                <View style={{ gap: spacing.sm }}>
                  <Text style={{ color: answer.correct ? colors.success : colors.danger, fontWeight: "900" }}>
